...
uint8_t       RX_buffer[FrameNum];  //接收的数据
uint8_t       TX_buffer[FrameNum];  //接收的数据
uint8_t       Txd_data = 0;//CAN总线要发送的数据，也是要在数码管1-2位置显示的数据
uint8_t       Rxd_data = 0;//CAN总线要接收的数据，也是要在数码管3-4位置显示的数据

void Peli_Init(uint32_t BaseAddr); // 初始化CAN总线芯片
void Peli_TXD(void);//CAN发送子函数


//#define SJA_BASE_ADR     CAN_0_BASE	 //寄存器地址的基址
